...
import { removeARequest } from "../utils/redux/pendingRequestsSlice";

const UserCard = ({ user, status1, status2, connectionId }) => {
  ...
  const handleReviewRequest = async (status, requestId) => {
    ...
    dispatch(removeARequest(requestId));
  };
    ...
    <div className="flex flex-col md:flex-row items-center bg-[#fff8f5] border border-[#f5e0d6] rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 w-full max-w-2xl overflow-hidden">
      ...
      <div className="w-full md:w-2/3 p-6 flex flex-col justify-between">
        ...
        <div className="flex gap-4">
          <button
            className="btn flex-1 bg-[#c53d0e] hover:bg-[#b3360c] border-none text-white transition-all duration-300"
            onClick={() => handleReviewRequest("accepted",connectionId)}
          >
            ❤️ {status1}
          </button>
          <button
            className="btn flex-1 btn-outline border-[#c53d0e] text-[#c53d0e] hover:bg-[#c53d0e] hover:text-white transition-all duration-300"
            onClick={() => handleReviewRequest("rejected",connectionId)}
          >
            🚫 {status2}
          </button>
        </div>
